...
export default async function handler(req: any, res: any) {
  ...
  // Accept a per-request key from the client (dev / preview only) — the
  // browser sends it as `x-eleven-key` so users can paste their key into
  // the preview banner without restarting vercel dev. In production this
  // header is normally absent and we fall back to the env var.
  const headerKey = (req.headers && (req.headers['x-eleven-key'] || req.headers['X-Eleven-Key'])) as string | undefined;
  const apiKey = (typeof headerKey === 'string' && headerKey.trim()) || process.env.ELEVENLABS_API_KEY;
  if (!apiKey) {
    return res.status(500).json({
      error: 'ELEVENLABS_API_KEY is not configured in Vercel.',
      hint: 'Project Settings → Environment Variables → add ELEVENLABS_API_KEY (Production), then redeploy. Or paste a key into the dev preview banner.',
      provider: 'elevenlabs',
    });
  }
