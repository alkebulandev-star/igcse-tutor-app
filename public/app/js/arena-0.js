/* ════════════════════════════════════════════════════════════════
   ...
   - Class groups: Kids · Juniors (JSS) · Seniors (SS) · Cambridge/A-Level / UCAS Prep
...
var CLASS_GROUPS = [
  { id:'kids',    name:'Kids',          sub:'Pre-Primary · Year 2–6', emoji:'🧒', accent:'#f59e0b' },
  { id:'juniors', name:'Juniors',       sub:'Year 7–3',                   emoji:'🎒', accent:'#3b82f6' },
  { id:'seniors', name:'Seniors',       sub:'SS 1–3',                    emoji:'🎓', accent:'#10b981' },
  { id:'prep',    name:'Exam Prep',     sub:'Cambridge · Edexcel · A-Level / UCAS',        emoji:'🏆', accent:'#a855f7' }
];

var SCOPES = [
  { id:'local',      name:'Local',      sub:'Your school / area',      emoji:'🏘️' },
  { id:'state',      name:'State',      sub:'Across your state',        emoji:'🗺️' },
  { id:'nationwide', name:'Nationwide', sub:'All the United Kingdom',              emoji:'🇬🇧' }
];
...
var STATES = ['Yorkshire','Norfolk','Hampshire','Cambridgeshire','Lancashire','Devon','Suffolk','Northumberland','Kent','Aberdeenshire','Surrey','Buckinghamshire','Sussex','Sheffield','Greater London - London','Cumbria','Essex','Cornwall','Leeds','Manchester','Dorset','Wiltshire','Somerset','Worcestershire','London','Warwickshire','Tyne','Berkshire','Oxfordshire','Cheshire','Hertfordshire','Derbyshire','Merseyside','Nottinghamshire','Staffordshire','Lincolnshire','Leicestershire'];
...
var GAMES = [
  ...
  },
  {
    ...
    desc:'Capitals, states, rivers — the United Kingdom & the world. Speed counts.',
...
function botName(){
  var n = ['Ada','Oliver','Lucas','Aaliyah','Edward','Florence','Isaac','Sophia','Yusuf','Zara','Isla','Oscar','Hannah','Amelia','Sienna','Mohammed'];
...
function injectPage(){
  ...
  pg.innerHTML = ''
    ...
    +   '<div class="ar-hero">'
    +     '<div class="ar-hero-l">'
    +       '<h2>Compete live. Win prizes. Top the leaderboard.</h2>'
    +       '<p>Educational games against real British-curriculum students worldwide. Pick your year group and play locally, nationally, or internationally. Weekend tournaments offer book vouchers, exam-board past-paper bundles and Scholar-plan upgrades.</p>'
    +     '</div>'
...
var ArenaUI = {
  open: function(){
    ...
    document.getElementById('arStateSelect').value = p.state || 'London';
  ...
  refresh: function(){
    ...
    var st = (state.scope === 'state') ? (getProfile().state || 'London') : null;
  ...
  editProfile: function(){
    ...
    m.innerHTML = ''
      ...
      +   '<div class="ar-form-row"><label>School (optional)</label><input id="arEditSchool" value="'+(p.school||'').replace(/"/g,'')+'" placeholder="e.g. Kings College London"/></div>'
...
function openLobby(room, game){
  ...
  var greetings = ['Hey hey 👋','GL everyone','UK stand up 🇬🇧','I\'m on mobile o','Quick match please','Anyone from London?','First time here','Let\'s warm up'];
...
function playQuizDuel(ctx){
  var BANK = {
    juniors: [
      { q:'Capital of the United Kingdom?', opts:['London','London','Birmingham','Manchester'], a:1 },
      { q:'How many states does the United Kingdom have?', opts:['30','35','36','37'], a:2 },
      { q:'Largest river in the United Kingdom?', opts:['Suffolk','Tyne','Cross','Berkshire'], a:1 },
    ...
    seniors: [
      ...
      { q:'1914 amalgamation joined?', opts:['North & South','East & West','Yoruba & Spanish','London & Manchester'], a:0 },
      { q:'Mitochondria function?', opts:['Photosynthesis','Energy','Storage','Reproduction'], a:1 }
    ],
    prep: [
      { q:'Differential of x³?', opts:['x²','3x²','3x','x³/3'], a:1 },
      { q:'Year the United Kingdom gained independence?', opts:['1957','1960','1963','1966'], a:1 },
      { q:'Avogadro\'s number ≈?', opts:['6.02×10²³','3.14','9.81','2.71'], a:0 },
      { q:'Speed of light ≈ (m/s)?', opts:['3×10⁵','3×10⁶','3×10⁷','3×10⁸'], a:3 },
      { q:'Currency of the United Kingdom?', opts:['Pound','Pounds','Franc','Rand'], a:1 }
    ],
    kids: [
...
function playGeoSprint(ctx){
  var QS = [
    { q:'Capital of London State?', opts:['London','Ikeja','Badagry','Epe'], a:1 },
    { q:'River Thames empties into?', opts:['Atlantic Ocean','Indian Ocean','Lake Chad','Mediterranean'], a:0 },
    { q:'Capital of France?', opts:['Lyon','Paris','Marseille','Nice'], a:1 },
    { q:'Largest desert in Africa?', opts:['Kalahari','Namib','Sahara','Gobi'], a:2 },
    { q:'Capital of Manchester State?', opts:['Manchester','Wudil','Bichi','Dawakin'], a:0 },
    { q:'Mt Kilimanjaro is in?', opts:['Kenya','Tanzania','Uganda','Ethiopia'], a:1 },
    { q:'Capital of Cambridgeshire State?', opts:['Onitsha','Awka','Nnewi','Aguata'], a:1 }
  ];
  return new Promise(function(resolve){
...
function openSpectator(room){
  ...
  var EMO = ['🔥','💯','😅','😎','🤯','👏','😂','🙌','🎯'];
  var BOT_CHATS = ['Up London!','Britain on top 💪','Who else watching?','That last one was tough','Easy money 🤑','GO GO GO!','Spelling Bee is hard though','Math Race is wild today'];
  ...
  var CHEAT_WORDS = ['buy','sell','pass','skip','don\'t buy','dont buy','mortgage','build','house','roll','rent','jail','pay',
    'manchester','birmingham','tube','sheffield','national-grid','birmingham2','leeds','london2','mayfair','unilever','newcastle','westminster','kensington',
...
function getQuestionBank(gameId, classGroup){
  ...
  var QUIZ = {
    juniors: [
      ...
      { q:'Author of Things Fall Apart?', opts:['Soyinka','Achebe','Adichie','Okri'], a:1 },
      { q:'H2O is the formula for?', opts:['Salt','Water','Acid','Air'], a:1 },
      { q:'First president of the United Kingdom?', opts:['Azikiwe','Balewa','Awolowo','Obasanjo'], a:0 },
