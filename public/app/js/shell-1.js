...
(function(){
  ...
  // Uses the LTMascot.mountButton helper for the mascot avatar + label. Falls
  // back to a plain pill if mascot module hasn't loaded yet.
  function mountPill(){
    if (document.getElementById('lt-srv-pill')) return;
    if (window.LTMascot && typeof window.LTMascot.mountButton === 'function'){
      var btn = window.LTMascot.mountButton({ label: pillLabel(), onClick: openServerMenu, dot: true });
      btn.id = 'lt-srv-pill';
      btn.title = 'Click to change AI server';
  ...
  function refreshPill(pill){
    ...
    if (!pill) return;
    // Mascot pill — update only the label span (second child)
    if (pill.classList && pill.classList.contains('lt-mascot-fab')){
      var span = pill.querySelector('span:not(.lt-fab-dot)');
      if (span) span.textContent = pillLabel();
      return;
    }
    var dot = window._ltLastProvider === 'openai' ? '🟢'
            : window._ltLastProvider === 'anthropic' ? '🔵'
            : window._ltLastProvider === 'both' ? '🟣' : '⚪';
    pill.innerHTML = '<span>'+dot+'</span><span style="font-weight:700">'+pillLabel()+'</span>';
  }
  ...
  function mountBanner(){
    ...
    bar.innerHTML =
      ...
      +'<input id="lt-el-input" type="password" placeholder="ELEVENLABS_API_KEY (British voice)" style="flex:1;min-width:140px;background:#fff;border:1px solid #fbbf24;color:#000;padding:5px 8px;border-radius:4px;font:12px monospace" />'
    ...
    var el = bar.querySelector('#lt-el-input');
    var st = bar.querySelector('#lt-key-status');
    ai.value = getKey(); oi.value = getOaiKey();
    try { el.value = localStorage.getItem('ELEVENLABS_API_KEY') || ''; } catch(_){}
    if (ai.value || oi.value || el.value) st.textContent = '✓ saved';
    bar.querySelector('#lt-key-save').onclick = function(){
      setKey(ai.value.trim()); setOaiKey(oi.value.trim());
      try { localStorage.setItem('ELEVENLABS_API_KEY', el.value.trim()); } catch(_){}
