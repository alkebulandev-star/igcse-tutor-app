...
// ════════════════════ INLINE MARKDOWN (module-scope) ════════════════════
// Lightweight inline markdown → HTML used by lesson renderer, callouts,
// exam mark schemes and any place AI output is injected into the DOM.
// **bold**, *italic*, _italic_, `code`, ~~strike~~, paragraphs from \n\n.
window.LTMd = (function(){
  function inline(s){
    if (!s) return '';
    var t = String(s);
    if (!/<\w+[\s>]/.test(t)){
    ...
    t = t.replace(/(^|[^\*\w])\*([^*\n]+)\*(?=[^\w]|$)/g, '$1<em>$2</em>');
    t = t.replace(/(^|[^_\w])_([^_\n]+)_(?=[^\w]|$)/g, '$1<em>$2</em>');
    t = t.replace(/`([^`\n]+)`/g, '<code style="background:rgba(15,23,42,.06);padding:1px 5px;border-radius:4px;font-family:ui-monospace,Menlo,monospace;font-size:.92em">$1</code>');
    t = t.replace(/~~([^~\n]+)~~/g, '<s>$1</s>');
    if (t.indexOf('\n\n') !== -1){
      t = t.split(/\n\n+/).map(function(p){ return p.replace(/\n/g,'<br>'); }).join('</p><p style="margin:.7em 0 0">');
...
(repeat the same format for all 5 questions, each followed by its own --- line)`;
  ...
  // Helper: extract a section between two markers, then apply inline
  // markdown so **bold**, *italic*, etc. render in the lesson HTML.
  function extract(text, marker){
    ...
    return (window.LTMd && window.LTMd.inline)
      ? window.LTMd.inline(text.substring(after, end).trim())
      : text.substring(after, end).trim();
  }
...
window.quizAnswer = function(btn, isCorrect){
  ...
  if(fb){
    var reteachBtnHTML = isCorrect ? '' :
      `<button class="lt-quiz-reteach" style="margin-top:10px;background:#264e36;color:#fbf8f1;border:none;padding:8px 14px;border-radius:100px;font-family:inherit;font-weight:700;font-size:.84rem;cursor:pointer;display:inline-flex;align-items:center;gap:6px">🎓 Re-teach me 1-on-1</button>`;
    ...
    if (!isCorrect){
      var rb = fb.querySelector('.lt-quiz-reteach');
      if (rb && window.LTReteach){
        rb.addEventListener('click', function(){
          var correct = (q.options || []).find(function(o){ return o.correct; });
          var chosenOpt = (q.options || []).find(function(o){ return o.text === btn.textContent || (btn.dataset && btn.dataset.idx == o.idx); });
          window.LTReteach.open({
            question: q.q || q.question || '',
            studentAnswer: chosenOpt ? chosenOpt.text : (btn.textContent || ''),
            correctAnswer: correct ? correct.text : '',
            explanation: q.explain || '',
            subject: (window.currentSubjectName || window.currentExamSubjName || 'this topic'),
            paperType: 'Lesson quiz'
          });
        });
      }
    }
...
// ── THEME TOGGLE ──
// Default theme is LIGHT (prospectus cream). User can flip to DARK via the
// 🌙 button; choice is persisted. The .dark-mode class on <html> drives the
// dark-theme overrides in theme-prospectus-1.css.
...
// Initialise on load so the chosen theme persists across navigation
...
function startLessonProgress(){
  ...
  let step = 0;
  let lastStepIdx = -1;
  // Mascot mood per step: 0=syllabus, 1=notes, 2=examples, 3=tips, 4=quiz
  const moodByStep = ['thinking','explaining','explaining','encouraging','encouraging'];
  function setLoaderMood(state){
    var host = document.getElementById('lessonLoaderMascot');
    if (!host || !window.LTMascot) return;
    if (!host.firstChild) window.LTMascot.render(host, { size: 80, state: state });
    else window.LTMascot.setState(host, state);
  }
  setLoaderMood('thinking');
  ...
  _progressInterval = setInterval(()=>{
    if(step < steps.length){
      ...
      setLoaderMood(moodByStep[si] || 'thinking');
...
function stopLessonProgress(){
  ...
  // Mascot — flip to encouraging on completion
  try {
    var host = document.getElementById('lessonLoaderMascot');
    if (host && window.LTMascot) window.LTMascot.setState(host, 'encouraging');
  } catch(e){}
  // Mark all steps done
  for(let i=0;i<5;i++){
    const el=document.getElementById('lsStep'+i);
...
function renderCurrentQuestion() {
  ...
  if(body) {
    ...
    // Reveal mark scheme + exemplar answer + examiner tip (Cambridge-style).
    // Hidden until the student opts in so it does not spoil practice.
    var tp = q.topic || {};
    if (tp.scheme || tp.exemplar || tp.examiner_tip){
      html += '<details class="tqc-scheme" style="margin-top:18px;border:1px solid var(--pp-line,#d8d2bf);border-radius:8px;background:#fdfaf2;">'
        + '<summary style="padding:12px 16px;cursor:pointer;font-weight:700;color:#264e36;list-style:none;display:flex;align-items:center;gap:8px;">'
          + '<span style="font-size:1.1rem">📋</span><span>Reveal mark scheme &amp; model answer</span>'
          + '<span style="margin-left:auto;font-size:.7rem;color:#8a5a14;letter-spacing:.06em;text-transform:uppercase">Click to expand</span>'
        + '</summary>'
        + '<div style="padding:14px 18px 18px;border-top:1px solid var(--pp-line,#d8d2bf);font-family:var(--pp-sans,system-ui);">';
      if (Array.isArray(tp.scheme) && tp.scheme.length){
        html += '<div style="font-size:.7rem;font-weight:800;color:#264e36;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px">Marking points</div>'
          + '<ol style="margin:0 0 14px 18px;padding:0;color:#142028;line-height:1.7;font-size:.92rem">'
          +   tp.scheme.map(function(s){ return '<li style="margin-bottom:4px">' + (window.LTMd ? window.LTMd.inline(s) : s) + '</li>'; }).join('')
          + '</ol>';
      }
      if (tp.exemplar){
        html += '<div style="font-size:.7rem;font-weight:800;color:#264e36;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px">Model answer (full marks)</div>'
          + '<div style="padding:10px 14px;background:#fff;border-left:3px solid #264e36;border-radius:0 6px 6px 0;color:#142028;line-height:1.7;font-size:.92rem;margin-bottom:14px;font-family:Georgia,serif">'
          +   (window.LTMd ? window.LTMd.inline(tp.exemplar) : tp.exemplar)
          + '</div>';
      }
      if (tp.examiner_tip){
        html += '<div style="display:flex;gap:8px;padding:10px 14px;background:#fdf3e0;border:1px solid #e0c97a;border-radius:6px;color:#6b4513;font-size:.86rem;line-height:1.55">'
          + '<span>🎯</span><span><strong>Examiner tip:</strong> ' + (window.LTMd ? window.LTMd.inline(tp.examiner_tip) : tp.examiner_tip) + '</span>'
          + '</div>';
      }
      html += '</div></details>';
    }
...
async function startExamSession(){
  ...
  if (adaptive && adaptive.prompt){
    ...
    prompt = 'Generate exactly '+batchSize+' ORIGINAL '+cfg.short+' '+subjName+' multiple-choice questions in the past-paper style.\n\n'
      ...
      +'• Past Cambridge/Edexcel papers are copyrighted. NEVER reproduce a past-paper question verbatim.\n'
      +'• Instead, write ORIGINAL questions that match the past-paper STYLE, COMMAND WORDS, DIFFICULTY, DISTRACTOR LOGIC and SYLLABUS COVERAGE.\n'
      +'• Reuse the question template (e.g. data-table → calculate → identify) but use fresh contexts, fresh numbers, fresh names every time.\n\n'
      +'=== MARK-SCHEME RIGOUR ===\n'
      +'• Align to Cambridge Assessment Objectives — AO1 knowledge, AO2 application, AO3 analysis & evaluation.\n'
      +'• Each "exp" must explain why the correct answer is right AND why each distractor is wrong (Cambridge "examiner commentary" style).\n'
      +'• Use Cambridge command words exactly: Define, State, Identify, Describe, Explain, Calculate, Compare, Justify.\n\n'
      +'=== STRICT FORMAT RULES ===\n'
      +'1. EVERY question MUST have EXACTLY '+cfg.options.length+' options: '+optLetters+'. All distractors must be plausible misconceptions a real student would make.\n'
      +'2. The "ans" field is exactly ONE letter from: '+optLetters+'.\n'
      +'3. Cover the full official '+cfg.short+' '+subjName+' syllabus — spread across major topics. Do not clump.\n'
      +'4. Return ONLY a raw JSON array — no markdown fences, no text outside the array.\n\n'
      +'=== OUTPUT FORMAT ===\n'
      +'[{"yr":2022,"topic":"topic name","q":"Question text","opts":{'+exampleOpts+'},"ans":"'+exampleAns+'","exp":"Why '+exampleAns+' is correct (assessment objective). Then: A is wrong because… B is wrong because… etc.","diff":"medium","ao":"AO2"}]';
  }
...
function submitExamAnswer(chosen){
  ...
  if(isPractice&&expEl){
    // 1-on-1 tutor mode: when ON, the AI coaching panel opens AUTOMATICALLY
    // after every wrong answer (no extra tap). The student also gets a
    // toggle to switch back to "show mark scheme only" mode.
    var oneOnOne = (function(){ try { return localStorage.getItem('lt_1on1_mode') !== '0'; } catch(_){ return true; }})();
    var reteachBtn = ok ? '' :
      '<button id="lt-rt-open-btn" type="button" style="margin-top:14px;background:#264e36;color:#fbf8f1;border:none;padding:10px 18px;border-radius:100px;font-family:inherit;font-weight:700;font-size:.92rem;cursor:pointer;display:inline-flex;align-items:center;gap:8px">'
        + '<span style="font-size:1.05rem">🎓</span>'
        + '<span>'+(oneOnOne?'Open 1-on-1 tutor':'Re-teach me — 1-on-1 with the tutor')+'</span>'
      + '</button>';
    var modeRow =
      '<label style="margin-top:14px;display:inline-flex;align-items:center;gap:8px;font-size:.78rem;color:#5b6370;background:#fff;border:1px solid var(--pp-line,#d8d2bf);padding:6px 12px;border-radius:100px;cursor:pointer;font-family:var(--pp-sans,system-ui)">'
        + '<input id="lt-1on1-toggle" type="checkbox"'+(oneOnOne?' checked':'')+' style="accent-color:#264e36">'
        + '<span><strong style="color:#264e36">1-on-1 mode</strong> — tutor opens automatically on wrong answers</span>'
      + '</label>';
    expEl.innerHTML='<div class="epb-exp">'
      ...
      +'</div>'
      +'</div>';
    // Wire the re-teach button and mode toggle.
    function openTutor(){
      if (!window.LTReteach) return;
      window.LTReteach.open({
        question: q.question,
        studentAnswer: '(' + chosen + ') ' + ((q.options && q.options[chosen]) || ''),
        correctAnswer: '(' + q.answer + ') ' + ((q.options && q.options[q.answer]) || ''),
        explanation: q.explanation,
        subject: examSession.subj || (window.currentExamSubjName || ''),
        board: (window._currentPaper && window._currentPaper.boardName) || (window.currentExam || '').toUpperCase(),
        paperType: 'Multiple Choice (MCQ)'
      });
    }
    if (!ok){
      var btn = document.getElementById('lt-rt-open-btn');
      if (btn) btn.addEventListener('click', openTutor);
      if (oneOnOne) setTimeout(openTutor, 420);
    }
    var tg = document.getElementById('lt-1on1-toggle');
    if (tg) tg.addEventListener('change', function(){
      try { localStorage.setItem('lt_1on1_mode', tg.checked ? '1' : '0'); } catch(_){}
    });
