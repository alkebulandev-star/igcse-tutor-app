/* ════════════════════════════════════════════════════════════════
   ...
   chosen French / Spanish / Mandarin / German automatically.
   ════════════════════════════════════════════════════════════════ */
(function(){
'use strict';
...
async function handleGenerate(){
  ...
  var board = (ctx.exam || resolveBoardFromUI() || 'IGCSE').toString().toUpperCase();
  var rawKey = ctx.key || '';
  ...
  if (board === 'SATS' || board === 'ELEVENPLUS' || board === '11+'){
    alert(board + ' is multiple-choice / short-answer only — there is no extended theory paper. Use Paper 1 instead.');
    return;
  }
...
function resolveBoardFromUI(){
  ...
  if (bc && bc.textContent) {
    ...
    if (/IGCSE|A-?LEVEL|ALEVEL|SATS|11\+|ELEVEN ?PLUS|EDEXCEL|CAMBRIDGE/.test(t)){
      if (/EDEXCEL/.test(t) && /A-?LEVEL/.test(t)) return 'EDXALEVEL';
      if (/EDEXCEL/.test(t)) return 'EDXIGCSE';
      if (/A-?LEVEL/.test(t)) return 'ALEVEL';
      if (/SATS|KS2/.test(t)) return 'SATS';
      if (/11\+|ELEVEN ?PLUS/.test(t)) return 'ELEVENPLUS';
      return 'IGCSE';
    }
  }
  var top = document.getElementById('ecTopTitle');
  if (top && top.textContent){
    var s = top.textContent.toUpperCase();
    if (/EDEXCEL/.test(s) && /A-?LEVEL/.test(s)) return 'EDXALEVEL';
    if (/EDEXCEL/.test(s)) return 'EDXIGCSE';
    if (/A-?LEVEL/.test(s)) return 'ALEVEL';
    if (/SATS|KS2/.test(s)) return 'SATS';
    if (/11\+|ELEVEN ?PLUS/.test(s)) return 'ELEVENPLUS';
    if (/IGCSE|CAMBRIDGE/.test(s)) return 'IGCSE';
  }
  return null;
}
